...
export default defineConfig(
  ...
  js.configs.recommended,
  {
    ...
    rules: {
      ...
      // Type-only imports must say so. Per-file transpilers (vite, vitest,
      // bun) erase types without a symbol table, so they cannot tell a type
      // import from a value import on their own. Autofixable.
      "@typescript-eslint/consistent-type-imports": [
        "error",
        { prefer: "type-imports", fixStyle: "separate-type-imports" },
      ],
      // A switch over a union must handle every member. Adding a variant to
      // the union then breaks the build at each switch that forgot it,
  ...
  // shadcn/ui components are generated by `npx shadcn add`, not authored here.
  // cva-based ones export their variants beside the component, which trips
  // react-refresh. Editing them is pointless - the next add overwrites it.
  {
